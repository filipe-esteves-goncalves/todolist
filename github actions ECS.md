...
Permissions: Must have packages: write and contents: read.
...
Trigger: After the publish workflow completes (workflow_run) or manually.
...
Wait for the app to be ready: Poll http://localhost:8081/actuator/health until status is UP.
...
Hardcoded Config: Since this is a public learning repo, use hardcoded strings for DB environment variables (DB_USER=myuser, etc.) directly in the workflow/task-def.

Task 3: ECS Task Definition Template

Goal: Provide a aws/task-definition.json for the agent to use.
...
Step 1: Generate .github/workflows/publish.yml.

Step 2: Generate aws/task-definition.json (replacing <OWNER> with the actual repo owner).

Step 3: Generate .github/workflows/smoke-test.yml. Use a simple run block for the curl commands.

Step 4: Important: Do not use custom secrets.* except for the built-in GITHUB_TOKEN. Hardcode all other values.
